...
import com.iwanvi.bookstore.book.job.utils.DateUtils;
...
public class BookIndexServiceImpl implements BookIndexService {
    ...
    @Autowired
    private BookDao bookDao;//书籍
    @Autowired
    private ChannelLibBookDao channelLibBookDao;//渠道书籍
    @Autowired
    private BookPvDao bookPvDao;//书籍pv统计
     ...
     * 需要过滤掉的书籍不能被搜索到
     */
    private static List<String>  FILTER_BOOK_LIST = new ArrayList<String>(){{
        add("23653");
        add("60360651");
        add("60748484");
    }};
...
//    private static BlockingQueue<List<ESBook>> Q = new LinkedBlockingDeque<>(500);
    ...
    private void setDateToQ(){
        ...
        //查询出来pv最近的一天
        String dt = queryMaxDt();
        //启动一个消费者线程，里边再启动线程池
        new Thread(new ConsumerRunner()).start();
        // 获取书城自有图书资源
        int id = 0;//book_v3表中的ID
        while (true) {
            //查询book，dt一般是昨天的日期，比如2017-11-21
            List<Map> resultList = bookDao.queryBookForSearchForMap(id, BATCH_SIZE, dt);
            try{
                //放入队列里边
    ...
    /**
     * 图书访问量最新更新时间（获取的id最大的条记录）
     * @author zzw
     * @since 2018年12月14日16:57:47
     */
    private String queryMaxDt() {
        String date = bookPvDao.queryMaxDt();
        LOGGER.info("book pv 的最新更新时间={}",date);
        return date;
    }
    ...
    private void initBuildIndex() {
        ...
        LOGGER.info("索引列表名称 indexList={}", GSON.toJson(indexList));
        //获取索引的下标
...
//            String workingIndexName = getWorkingIndexName();
    ...
    private String getWorkingIndexName() {
        try {
            ...
            LOGGER.info("获取当前索引返回={}", GSON.toJson(result));
            //获取索引的别名映射
            ObjectMapper objectMapper = new ObjectMapper();
            List<Map> aliasMaps = objectMapper.readValue(result.getJsonString(), List.class);
            LOGGER.info("aliasMaps={}", GSON.toJson(aliasMaps));
 ...
 * @since 2018年12月14日16:57:47
 */
private static final int consumer_pool_size = 9;//消费线程池大小
...
//            if (FILTER_BOOK_LIST.contains(indexId)) {
//                continue;
//            }
            // 获取最近更新时间,book_v3表更新时间维护正确后,后续可以去掉此逻辑
            long bookUpdateTime = 0l;
            long chapUpdateTime = 0l;
            //为啥要移除书籍更新时间
            if (map.get("bookUpdateTime") != null) {
                bookUpdateTime = ((Date) map.get("bookUpdateTime")).getTime();
                map.remove("bookUpdateTime");
            }
            //移除最后个章节更新时间
            String lastUpdateChapterDateTemp = String.valueOf(map.get("lastUpdateChapterDate"));
            if (Utils.isNotEmpty(lastUpdateChapterDateTemp)) {
                try {
                    chapUpdateTime = DateUtils.dateParse(lastUpdateChapterDateTemp, DateUtils.Y4MDHMS).getTime();
                } catch (Exception e) {
                    LOGGER.error("最后更新时间格式化异常 bookId={} lastUpdateChapterDate={} {}", indexId,lastUpdateChapterDateTemp,e);
                }
                map.remove("lastUpdateChapterDate");
            }else{
                LOGGER.warn("图书的最新更新时间为空 bookId={}",indexId);
            }
            //移除id，这个又是为了es自己生成主键？
            map.remove("id");
            //最后补充一个更新时间
            map.put("updateTime", bookUpdateTime > chapUpdateTime ? bookUpdateTime : chapUpdateTime);
            //获取书籍渠道id=
